...
app.post('/create-payment-intent', paymentLimiter, [
  body('service').isString().isIn(['website', 'ecommerce', 'consultation']).trim(),
  ...
  try {
    ...
    if (!errors.isEmpty()) {
      return res.status(400).json({ error: 'Invalid input parameters', details: errors.array() });
    }
...
app.post('/webhook', paymentLimiter, express.raw({ type: 'application/json' }), (req, res) => {
